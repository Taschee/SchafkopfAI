...
class TestGameMode:
    ...
    def test_solo_beats_partner_mode(self):
    ...
    def test_wenz_beats_no_game(self):
        wenz = GameMode(game_type=WENZ, suit=NO_SUIT)
        no_game = GameMode(game_type=NO_GAME, suit=NO_SUIT)
        assert wenz > no_game
        assert no_game < wenz

    def test_partner_mode_beats_no_game(self):
        partner_game = GameMode(game_type=PARTNER_MODE, suit=random.choice(SUITS_WITHOUT_HEARTS))
        no_game = GameMode(game_type=NO_GAME, suit=NO_SUIT)
    ...
    def test_illegal_suit_for_game_type_raises_exception(self):
